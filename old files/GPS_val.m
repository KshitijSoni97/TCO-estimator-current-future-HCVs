...
% clc
% ValDat = xlsread('C:\Users\Kshitij\Desktop\Thesis docs\Code\Vehicle 2 - Class 5 TnT\Class5_ tractor_Long_Haul+ES_1Hz.csv');
% ValTime = ValDat(:,1);
% ValDist = ValDat(:,3);
% ValVel = ValDat(:,4);
% ValTarg = ValDat(:,5);
...
R = readmatrix('Munich to Leipheim.csv');                   % Replace and read the saved csv file as a table
Rrep = R(:,7)==0;                                            % Deleting the repeated waypoints
R(Rrep,:) = [];
R(isnan(R)) = 0;                                            % Replacing NaN values with 0

Stoptimes(:,1) = [ 5000, 10000, 13000, 18036 ];         %% Input all stop locations including the end stop
Stoptimes(:,2) = [ 10, 10, 10, 10 ];                   %% Input all stop durations
...
%% Filtering the TSP according to a factor between 50-80 meters
s = divisors(length(MaxSpeedProfile));              % Finding a factor to smoothen the TSP
FMSP = MaxSpeedProfile;
for i = 1: length(s)
    if s(i) >= 50 && s(i) <= 80
        sect = s(i);
    end
end
for i = 1:sect:length(MaxSpeedProfile)
    for j = i
        FMSP(j:j+(sect-1)) = min(MaxSpeedProfile(i:i+(sect-1)));
    end
end
...
% out = sim('Refined_Model_42_VectoTnT.slx',1800);    
...
% plot(ValTime,(ValVel/3.6),'r','Linewidth',1.5)
% hold on
% hold on
% plot(ValTime,(ValTarg/3.6),'k','Linewidth',1.5)
% grid on
